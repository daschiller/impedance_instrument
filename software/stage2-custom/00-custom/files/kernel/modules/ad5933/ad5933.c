...
#define AD5933_EXT_OSC_FREQ_Hz		9000000
...
static int ad5933_change_clock(struct ad5933_state *st, unsigned long clock) {
	/*
	 * We have to toggle the ENAB bit in the control register to ensure
	 * that the current divider settings actually apply.
	 * This is an ugly workaround!
	 */
#define CM_GP0CTL 0xfe101070
	void __iomem *reg = ioremap(CM_GP0CTL, 4);
	u32 val;

	// printk(KERN_DEBUG "ad5933: clock = %lu\n", clock);
	val = readl(reg);
	writel(0x5a000000 | (val & ~(1 << 4)), reg);
	while(readl(reg) & (1 << 7)) {
		;
	}
	clk_set_rate(st->mclk, clock);
	st->mclk_hz = clk_get_rate(st->mclk);
	val = readl(reg);
	writel(0x5a000000 | val | (1 << 4), reg);
    iounmap(reg);
...
static ssize_t ad5933_show_clock(struct device *dev,
			   struct device_attribute *attr,
			   char *buf)
{
	struct iio_dev *indio_dev = dev_to_iio_dev(dev);
...
static ssize_t ad5933_store_clock(struct device *dev,
			    struct device_attribute *attr,
			    const char *buf,
			    size_t len)
{
	...
	ret = kstrtoul(buf, 0, &clock);
	if (ret) {
		return ret;
	}
	ad5933_change_clock(st, clock);
...
static IIO_DEVICE_ATTR(clock_frequency, 0644,
			ad5933_show_clock,
			ad5933_store_clock,
...
static struct attribute *ad5933_attributes[] = {
	...
	&iio_dev_attr_clock_frequency.dev_attr.attr,
	NULL
};
...
static void ad5933_work(struct work_struct *work)
{
	...
	if (st->state == AD5933_CTRL_INIT_START_FREQ) {
		...
		// printk(KERN_DEBUG "ad5933: starting sweep at %u Hz\n", st->freq_start);
	...
	if (status & AD5933_STAT_SWEEP_DONE) {
		...
		// printk(KERN_DEBUG "ad5933: incrementing frequency\n");
		schedule_delayed_work(&st->work, st->poll_time_jiffies);
	}
}
			...
			const struct i2c_device_id *id)
{
	...
	if (!IS_ERR(st->mclk)) {
		...
		ret = ad5933_change_clock(st, AD5933_EXT_OSC_FREQ_Hz);
		if (ret < 0)
			return dev_err_probe(&client->dev, ret, "failed to change clock frequency\n");
